...
DRUG_a5PAM = 0 # value is a dose multiplier factor - ignored if 0
DRUG_benzo = 0 # value is a dose multiplier factor - ignored if 0
...
if MDD:
	...
	# AGM - added to carry over changes for printing/DRUG code
	for post in cell_names:
		for pre in cell_names:
			circuit_params["syn_cond"].at[pre, post] = circuit_params["syn_params"][pre+post]["gmax"]

if DRUG_a5PAM>0:
	# Load new copy of original params because MDD changes params used before calculating factor
	original_params = pd.read_excel('Circuit_param.xls', sheet_name = None, index_col = 0)
	
	# synaptic enhancement
	for post in cell_names:
		for pre in cell_names:
			if (('PYR' in post) & ('SST' in pre)):
				factor = original_params['SING_CELL_PARAM'].at['drug_apic_tonic',post]/original_params['SING_CELL_PARAM'].at['apic_tonic',post]
				# expression_factor = 1 + (original_params['SING_CELL_PARAM'].at['p_GABRA5',post]/100)*(factor - 1) # expression_factor = 1 if apic_tonic = drug_apic_tonic or if p_GABRA5 = 0
				factor = 1 + (factor-1)*DRUG_a5PAM # dose
				circuit_params['syn_cond'].at[pre, post] = circuit_params['syn_cond'].at[pre, post]*factor
				circuit_params['syn_params'][pre+post]['gmax'] = circuit_params['syn_cond'].at[pre, post]
	
	# tonic enhancement
	for post in cell_names:
		if 'PYR' in post:
			factor = original_params['SING_CELL_PARAM'].at['drug_apic_tonic',post]/original_params['SING_CELL_PARAM'].at['apic_tonic',post] # factor by which to increase tonic due to drug in MDD case (based on increase in healthy)
			# expression_factor = 1 + (original_params['SING_CELL_PARAM'].at['p_GABRA5',post]/100)*(factor - 1) # expression_factor = 1 if apic_tonic = drug_apic_tonic or if p_GABRA5 = 0
			factor = 1 + (factor-1)*DRUG_a5PAM # dose
			circuit_params['SING_CELL_PARAM'].at['apic_tonic',post] = circuit_params['SING_CELL_PARAM'].at['apic_tonic',post]*factor
	...
	for post in cell_names:
		for pre in cell_names:
			if (('PYR' in post) & ('PYR' not in pre)):
				factor = original_params['SING_CELL_PARAM'].at['drug_apic_tonic',post]/original_params['SING_CELL_PARAM'].at['apic_tonic',post]
				# expression_factor = factor # because benzodiazepines are non-specific and 100% of cells should be affected
				factor = 1 + (factor-1)*DRUG_benzo # dose
				circuit_params['syn_cond'].at[pre, post] = circuit_params['syn_cond'].at[pre, post]*factor
				circuit_params['syn_params'][pre+post]['gmax'] = circuit_params['syn_cond'].at[pre, post]
			elif ('PYR' not in pre):
				factor = original_params['SING_CELL_PARAM'].at['drug_tonic',post]/original_params['SING_CELL_PARAM'].at['norm_tonic',post]
				# expression_factor = factor # because benzodiazepines are non-specific and 100% of cells should be affected
				factor = 1 + (factor-1)*DRUG_benzo # dose
				circuit_params['syn_cond'].at[pre, post] = circuit_params['syn_cond'].at[pre, post]*factor
				circuit_params['syn_params'][pre+post]['gmax'] = circuit_params['syn_cond'].at[pre, post]
	...
	for post in cell_names:
		if 'PYR' in post:
			...
			factor = 1 + (factor-1)*DRUG_benzo # dose
			circuit_params['SING_CELL_PARAM'].at['apic_tonic',post] = circuit_params['SING_CELL_PARAM'].at['apic_tonic',post]*factor
			circuit_params['SING_CELL_PARAM'].at['norm_tonic',post] = circuit_params['SING_CELL_PARAM'].at['norm_tonic',post]*factor
		else:
			factor = original_params['SING_CELL_PARAM'].at['drug_tonic',post]/original_params['SING_CELL_PARAM'].at['norm_tonic',post] # factor by which to increase tonic due to drug in MDD case (based on increase in healthy)
			# expression_factor = factor # because benzodiazepines are non-specific and 100% of cells should be affected
			factor = 1 + (factor-1)*DRUG_benzo # dose
			circuit_params['SING_CELL_PARAM'].at['norm_tonic',post] = circuit_params['SING_CELL_PARAM'].at['norm_tonic',post]*factor
...
pp.pprint(circuit_params['SING_CELL_PARAM']) if RANK==0 else None
pp.pprint(circuit_params['syn_cond']) if RANK==0 else None
